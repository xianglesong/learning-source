...
/**
 * A {@link ChannelUpstreamHandler} which provides an individual handler method for each event type.
 * This handler down-casts the received upstream event into more meaningful sub-type event and calls
 * an appropriate handler method with the down-cast event. The names of the methods are identical to
 * the upstream event names, as introduced in the {@link ChannelUpstreamHandler} documentation.
 * <p>
 ...
 * method just like overriding an ordinary Java method. Please make sure to call
 * {@code super.handleUpstream()} so that other handler methods are invoked properly:
 * </p>
 * 
 * <pre>
...
public class SimpleChannelUpstreamHandler implements ChannelUpstreamHandler {
  ...
  private static final InternalLogger logger = InternalLoggerFactory
      .getInstance(SimpleChannelUpstreamHandler.class.getName());
  ...
  public void handleUpstream(ChannelHandlerContext ctx, ChannelEvent e) throws Exception {
    if (e instanceof MessageEvent) {
      logger.info("messageEvent: " + ((MessageEvent) e).getMessage().toString());
      messageReceived(ctx, (MessageEvent) e);
    } else if (e instanceof WriteCompletionEvent) {
      logger.info("WriteCompletionEvent: " + ((MessageEvent) e).getMessage().toString());
      WriteCompletionEvent evt = (WriteCompletionEvent) e;
      writeComplete(ctx, evt);
    } else if (e instanceof ChildChannelStateEvent) {
      logger.info("ChildChannelStateEvent: " + ((MessageEvent) e).getMessage().toString());
      ...
      logger.info("ChannelStateEvent: " + ((MessageEvent) e).getMessage().toString());
  ...
  /**
   * Invoked when a {@link Channel} is open, bound to a local address, and connected to a remote
   * address.
   */
