...
/**
 * A random and sequential accessible sequence of zero or more bytes (octets). This interface
 * provides an abstract view for one or more primitive byte arrays ({@code byte[]}) and
 * {@linkplain ByteBuffer NIO buffers}.
 *
 * <h3>Creation of a buffer</h3>
 *
 * It is recommended to create a new buffer using the helper methods in {@link ChannelBuffers}
 ...
 * href="http://en.wikipedia.org/wiki/Index_(information_technology)#Array_element_identifier"
 * >zero-based indexing</a>. It means the index of the first byte is always {@code 0} and the index
 * of the last byte is always {@link #capacity() capacity - 1}. For example, to iterate all bytes of
 * a buffer, you can do the following, regardless of its internal implementation:
 *
 * <pre>
 ...
 *
 * {@link ChannelBuffer} provides two pointer variables to support sequential read and write
 * operations - {@link #readerIndex() readerIndex} for a read operation and {@link #writerIndex()
 * writerIndex} for a write operation respectively. The following diagram shows how a buffer is
 * segmented into three areas by the two pointers:
 *
 * <pre>
 *      +-------------------+------------------+------------------+
 ...
 * {@code read} or {@code skip} will get or skip the data at the current {@link #readerIndex()
 * readerIndex} and increase it by the number of read bytes. If the argument of the read operation
 * is also a {@link ChannelBuffer} and no destination index is specified, the specified buffer's
 * {@link #readerIndex() readerIndex} is increased together.
 * <p>
 * If there's not enough content left, {@link IndexOutOfBoundsException} is raised. The default
 * value of newly allocated, wrapped or copied buffer's {@link #readerIndex() readerIndex} is
 * {@code 0}.
 *
 * <pre>
 ...
 * {@code write} will write the data at the current {@link #writerIndex() writerIndex} and increase
 * it by the number of written bytes. If the argument of the write operation is also a
 * {@link ChannelBuffer}, and no source index is specified, the specified buffer's
 * {@link #readerIndex() readerIndex} is increased together.
 * <p>
 * If there's not enough writable bytes left, {@link IndexOutOfBoundsException} is raised. The
 * default value of newly allocated buffer's {@link #writerIndex() writerIndex} is {@code 0}. The
 * default value of wrapped or copied buffer's {@link #writerIndex() writerIndex} is the
 ...
 *
 * This segment contains the bytes which were read already by a read operation. Initially, the size
 * of this segment is {@code 0}, but its size increases up to the {@link #writerIndex() writerIndex}
 * as read operations are executed. The read bytes can be discarded by calling
 * {@link #discardReadBytes()} to reclaim unused area as depicted by the following diagram:
 *
 * <pre>
 ...
 *
 * You can set both {@link #readerIndex() readerIndex} and {@link #writerIndex() writerIndex} to
 * {@code 0} by calling {@link #clear()}. It does not clear the buffer content (e.g. filling with
 * {@code 0}) but just clears the two pointers. Please also note that the semantic of this operation
 ...
 *
 * Various {@code indexOf()} methods help you locate an index of a value which meets a certain
 * criteria. Complicated dynamic sequential search can be done with {@link ChannelBufferIndexFinder}
 * as well as simple static single byte search.
 *
 * <h3>Mark and reset</h3>
 *
 * There are two marker indexes in every buffer. One is for storing {@link #readerIndex()
 * readerIndex} and the other is for storing {@link #writerIndex() writerIndex}. You can always
 * reposition one of the two indexes by calling a reset method. It works in a similar fashion to the
 * mark and reset methods in {@link InputStream} except that there's no {@code readlimit}.
 *
 * <h3>Derived buffers</h3>
 *
 * You can create a view of an existing buffer by calling either {@link #duplicate()},
 * {@link #slice()} or {@link #slice(int, int)}. A derived buffer will have an independent
 * {@link #readerIndex() readerIndex}, {@link #writerIndex() writerIndex} and marker indexes, while
 * it shares other internal data representation, just like a NIO buffer does.
 * <p>
 * In case a completely fresh copy of an existing buffer is required, please call {@link #copy()}
 * method instead.
 *
 ...
 * <h4>NIO Buffers</h4>
 *
 * Various {@link #toByteBuffer()} and {@link #toByteBuffers()} methods convert a
 * {@link ChannelBuffer} into one or more NIO buffers. These methods avoid buffer allocation and
 * memory copy whenever possible, but there's no guarantee that memory copy will not be involved or
 * that an explicit memory copy will be involved.
 *
 * <h4>Strings</h4>
 *
 * Various {@link #toString(String)} methods convert a {@link ChannelBuffer} into a {@link String}.
 * Please note that {@link #toString()} is not a conversion method.
 *
 * <h4>I/O Streams</h4>
 *
...
public interface ChannelBuffer extends Comparable<ChannelBuffer> {
  ...
  /**
   * Returns the factory which creates a {@link ChannelBuffer} whose type and default
   * {@link ByteOrder} are same with this buffer.
   */
   ...
   *
   * @throws IndexOutOfBoundsException if the specified {@code writerIndex} is less than
   *         {@code this.readerIndex} or greater than {@code this.capacity}
   */
  void writerIndex(int writerIndex);

  /**
   * Sets the {@code readerIndex} and {@code writerIndex} of this buffer in one shot. This method is
   * useful when you have to worry about the invocation order of {@link #readerIndex(int)} and
   * {@link #writerIndex(int)} methods. For example, the following code will fail:
   *
   * <pre>
   ...
   * </pre>
   *
   * By contrast, {@link #setIndex(int, int)} guarantees that it never throws an
   * {@link IndexOutOfBoundsException} as long as the specified indexes meet basic constraints,
   * regardless what the current index values of the buffer are:
   *
   * <pre>
   ...
   *         specified {@code writerIndex} is less than the specified {@code readerIndex} or if the
  ...
  /**
   * Marks the current {@code readerIndex} in this buffer. You can reposition the current
   * {@code readerIndex} to the marked {@code readerIndex} by calling {@link #resetReaderIndex()}.
  ...
  /**
   * Marks the current {@code writerIndex} in this buffer. You can reposition the current
   * {@code writerIndex} to the marked {@code writerIndex} by calling {@link #resetWriterIndex()}.
   ...
   * {@code readerIndex} and {@code writerIndex} to the 0th index, and sets {@code readerIndex} and
   * {@code writerIndex} to {@code 0} and {@code oldWriterIndex - oldReaderIndex} respectively.
   * <p>
  ...
  /**
   * Gets a 24-bit medium integer at the specified absolute {@code index} in this buffer. This
   ...
   * {@link #getBytes(int, ChannelBuffer, int, int)}, except that this method increases the
   * {@code writerIndex} of the destination by the number of the transferred bytes while
   * {@link #getBytes(int, ChannelBuffer, int, int)} does not. This method does not modify
   * {@code readerIndex} or {@code writerIndex} of the source buffer (i.e. {@code this}).
   *
  ...
  /**
   * Transfers this buffer's data to the specified destination starting at the specified absolute
   * {@code index}. This method is basically same with
   * {@link #getBytes(int, ChannelBuffer, int, int)}, except that this method increases the
   * {@code writerIndex} of the destination by the number of the transferred bytes while
   * {@link #getBytes(int, ChannelBuffer, int, int)} does not. This method does not modify
   * {@code readerIndex} or {@code writerIndex} of the source buffer (i.e. {@code this}).
   ...
   * {@code readerIndex} or {@code writerIndex} of this buffer while the destination's
  ...
  /**
   * Sets the specified byte at the specified absolute {@code index} in this buffer. This method
   * does not modify {@code readerIndex} or {@code writerIndex} of this buffer.
   *
   * @throws IndexOutOfBoundsException if the specified {@code index} is less than {@code 0} or
   *         {@code index + 1} is greater than {@code this.capacity}
   ...
   * {@code index} until the destination becomes unreadable. This method is basically same with
   * {@link #setBytes(int, ChannelBuffer, int, int)}, except that this method increases the
   * {@code readerIndex} of the source buffer by the number of the transferred bytes while
   * {@link #getBytes(int, ChannelBuffer, int, int)} does not. This method does not modify
   * {@code readerIndex} or {@code writerIndex} of the source buffer (i.e. {@code this}).
   *
   * @throws IndexOutOfBoundsException if the specified {@code index} is less than {@code 0} or if
  ...
  /**
   * Transfers the specified source buffer's data to this buffer starting at the specified absolute
   * {@code index} until the source buffer's position reaches its limit. This method does not modify
   * {@code readerIndex} or {@code writerIndex} of this buffer.
   *
   * @throws IndexOutOfBoundsException if the specified {@code index} is less than {@code 0} or if
   ...
   * absolute {@code index}. This method does not modify {@code readerIndex} or {@code writerIndex}
   * of this buffer.
   *
   * @param length the number of bytes to transfer
   *
  ...
  /**
   * Gets a 16-bit short integer at the current {@code readerIndex} and increases the
  ...
  /**
   * Gets a 24-bit medium integer at the current {@code readerIndex} and increases the
  ...
  /**
   * Gets an unsigned 32-bit integer at the current {@code readerIndex} and increases the
   ...
   * {@code readerIndex} and increases the {@code readerIndex} by the number of the transferred
   * bytes (= {@code length}). The returned buffer's {@code readerIndex} and {@code writerIndex} are
  ...
  /**
   * Transfers this buffer's data to a newly created buffer starting at the current
   * {@code readerIndex} until the specified {@code indexFinder} returns {@code true} and increases
   * the {@code readerIndex} by the number of the transferred bytes. The returned buffer's
   * {@code readerIndex} and {@code writerIndex} are {@code 0} and the number of the transferred
   * bytes respectively.
   *
   ...
   * @return the newly created buffer which contains the transferred bytes
   *
   * @throws NoSuchElementException if {@code indexFinder} didn't return {@code true} at all
   */
  ChannelBuffer readBytes(ChannelBufferIndexFinder indexFinder);

  /**
   * Returns a new slice of this buffer's sub-region starting at the current {@code readerIndex} and
  ...
  /**
   * Returns a new slice of this buffer's sub-region starting at the current {@code readerIndex} and
   * increases the {@code readerIndex} by the size of the new slice (determined by
   * {@code indexFinder}).
   *
   * @param indexFinder finds the end index of the sub-region
   *
   ...
   * {@code readerIndex} until the destination becomes non-writable, and increases the
   * {@code readerIndex} by the number of the transferred bytes. This method is basically same with
   * {@link #readBytes(ChannelBuffer, int, int)}, except that this method increases the
   * {@code writerIndex} of the destination by the number of the transferred bytes while
   ...
   * {@link #readBytes(ChannelBuffer, int, int)}, except that this method increases the
   * {@code writerIndex} of the destination by the number of the transferred bytes (= {@code length}
   ...
   *         {@code length} is greater than {@code this.readableBytes}, or if
  ...
  /**
   * Transfers this buffer's data to the specified destination starting at the current
   * {@code readerIndex} until the destination's position reaches its limit, and increases the
   * {@code readerIndex} by the number of the transferred bytes.
   *
  ...
  /**
   * Increases the current {@code readerIndex} until the specified {@code indexFinder} returns
   * {@code true} in this buffer.
   *
   * @return the number of skipped bytes
   *
   * @throws NoSuchElementException if {@code firstIndexFinder} didn't return {@code true} at all
   */
   ...
   * {@code writerIndex} by {@code 1} in this buffer.
   *
   ...
   * {@link #writeBytes(ChannelBuffer, int, int)}, except that this method increases the
   * {@code readerIndex} of the source buffer by the number of the transferred bytes while
   ...
   * bytes (= {@code length}). This method is basically same with
   * {@link #writeBytes(ChannelBuffer, int, int)}, except that this method increases the
   * {@code readerIndex} of the source buffer by the number of the transferred bytes (=
   * {@code length}) while {@link #writeBytes(ChannelBuffer, int, int)} does not.
   *
   * @param length the number of bytes to transfer
   *
   ...
   * (exclusive).
   * <p>
   * If {@code fromIndex} is greater than {@code toIndex}, the search is performed in a reversed
   * order.
   * <p>
   * This method does not modify {@code readerIndex} or {@code writerIndex} of this buffer.
   *
  ...
  /**
   * Locates the first index where the specified {@code indexFinder} returns {@code true}. The
   * search takes place from the specified {@code fromIndex} (inclusive) to the specified
   ...
   * This method does not modify {@code readerIndex} or {@code writerIndex} of this buffer.
   *
   * @return the absolute index where the specified {@code indexFinder} returned {@code true} if the
   *         {@code indexFinder} returned {@code true}. {@code -1} otherwise.
   */
   ...
   * this buffer does not affect each other at all. This method is identical to
   * {@code buf.copy(buf.readerIndex(), buf.readableBytes())}. This method does not modify
   * {@code readerIndex} or {@code writerIndex} of this buffer.
   *
   */
  ChannelBuffer copy();
   ...
   * indexes and marks. This method is identical to {@code buf.slice(0, buf.capacity())}. This
   * method does not modify {@code readerIndex} or {@code writerIndex} of this buffer.
   */
  ChannelBuffer duplicate();

  /**
   * Converts this buffer's readable bytes into a NIO buffer. The returned buffer might or might not
   * share the content with this buffer, while they have separate indexes and marks. This method is
   * identical to {@code buf.toByteBuffer(buf.readerIndex(), buf.readableBytes())}. This method does
   * not modify {@code readerIndex} or {@code writerIndex} of this buffer.
   */
  ByteBuffer toByteBuffer();

  /**
   * Converts this buffer's sub-region into a NIO buffer. The returned buffer might or might not
   * share the content with this buffer, while they have separate indexes and marks. This method
   * does not modify {@code readerIndex} or {@code writerIndex} of this buffer.
   */
  ByteBuffer toByteBuffer(int index, int length);

  /**
   * Converts this buffer's readable bytes into an array of NIO buffers. The returned buffers might
   * or might not share the content with this buffer, while they have separate indexes and marks.
   * This method is identical to {@code buf.toByteBuffers(buf.readerIndex(), buf.readableBytes())}.
   * This method does not modify {@code readerIndex} or {@code writerIndex} of this buffer.
   */
  ByteBuffer[] toByteBuffers();

  /**
   * Converts this buffer's sub-region into an array of NIO buffers. The returned buffers might or
   * might not share the content with this buffer, while they have separate indexes and marks. This
   * method does not modify {@code readerIndex} or {@code writerIndex} of this buffer.
   */
  ...
  /**
   * Decodes this buffer's readable bytes into a string until the specified {@code terminatorFinder}
   * returns {@code true} with the specified character set name. This method is identical to
  ...
  /**
   * Decodes this buffer's sub-region into a string with the specified character set name. This
   * method does not modify {@code readerIndex} or {@code writerIndex} of this buffer.
   *
   * @throws UnsupportedCharsetException if the specified character set name is not supported by the
   *         current VM
  ...
  /**
   * Decodes this buffer's readable bytes into a string until the specified {@code terminatorFinder}
   * returns {@code true} with the specified character set name. This method does not modify
   * {@code readerIndex} or {@code writerIndex} of this buffer.
   *
   * @throws UnsupportedCharsetException if the specified character set name is not supported by the
   *         current VM
   ...
   * array which is {@linkplain #equals(Object) equal to} this array, both arrays should return the
   * same value.
   */
  int hashCode();
   ...
   * performed in the same manner with the string comparison functions of various languages such as
   * {@code strcmp}, {@code memcmp} and {@link String#compareTo(String)}.
   */
   ...
   * whole content of the buffer but returns the values of the key properties such as
   * {@link #readerIndex()}, {@link #writerIndex()} and {@link #capacity()}.
   */
  String toString();
}
